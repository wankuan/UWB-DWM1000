 ...
 *
 *           This is the same as the "TX then wait for response" example code with GPIOs and LEDs configured to help monitor TX and RX activity.
 *
 * @attention
 *
...
#define APP_NAME "TX W4R LED v1.0"
 ...
 * 6. TX and RX activity can be monitored using the following GPIOs (e.g. connected to an oscilloscope):
 ...
 * 8. dwt_writetxdata() takes the full size of tx_msg as a parameter but only copies (size - 2) bytes as the check-sum at the end of the frame is
