...
void SPI1_Init(void)
{
    ...
    SPI1_Handler.Instance=SPI2;                         //SP1
    ...
    //{   
						...
						{						
								num=*p-' ';
									for(n=0;n<8;n++)
								{
											OLED_GRAM[2*y][8*(x-1)+n]=ASCII[num][n];
								}	
								for(n=0;n<8;n++)
								{
											OLED_GRAM[2*y+1][8*(x-1)+n]=ASCII[num][n+8];
								}
								x++;
								p++;
...
void OLED_OUT_Num(u8 x,u8 y,u32 num,u8 size)
{
	...
	y=4-y;
	    for(i=0;i<size;i++)
	{
		buf_num[i]=(num/Math_th(10,size-i-1))%10;
	}
						for(i=0;i<size;i++)
						{						
										...
										for(n=0;n<8;n++)
									{
												OLED_GRAM[2*y][8*(x-1)+n]=ASCII[16+buf_num[i]][n];
									}	
									for(n=0;n<8;n++)
									{
												OLED_GRAM[2*y+1][8*(x-1)+n]=ASCII[16+buf_num[i]][n+8];
									}		
								x++;
					}  
}
